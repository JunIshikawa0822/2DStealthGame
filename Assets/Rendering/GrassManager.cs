...
public class GrassChunkManager : MonoBehaviour {
    ...
    /// 破棄完了後の区域を保持するプール
    /// </summary>
    private UnityEngine.Rendering.ObjectPool<GrassChunk> m_ChunkPool;
    ...
    private void Awake() {
        ...
        m_ChunkPool = new UnityEngine.Rendering.ObjectPool<GrassChunk>(null, null);
    ...
    private void LoadChunk() {
        ...
        GrassChunk grassGroup = m_LoadRequestQueue.Dequeue();
    ...
    private void UpdateMinMax(ref Vector3 minPos, ref Vector3 maxPos, Vector3 pos, Vector3 minOffset,
        ...
        Gizmos.color = Color.red; // バウンディングボックスの色を設定
        Gizmos.DrawWireCube((minPos + maxPos) / 2, maxPos - minPos); // バウンディングボックスの中心とサイズを指定
    }

    // private void OnDrawGizmos()
    // {
    //     Gizmos.color = Color.green; // バウンディングボックスの色を設定
    //     Gizmos.DrawWireCube((minPos + maxPos) / 2, maxPos - minPos); // バウンディングボックスの中心とサイズを指定
    // }
