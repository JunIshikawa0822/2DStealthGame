...
public static class GrassChunkDataUtility {
        ...
        public static async UniTask<GrassChunkGenerateData> LoadGrassChunkAsync(Vector2Int chunkID, CancellationToken ct) {
            ...
            try {
                byte[] dataBytes = await LoadDataBytesAsync(chunkID, ct);
                if (dataBytes == null) {
                    return null;
                }

                int intByteSize = ConstParam.Int32ByteSize; //4
                int vector3ByteSize = ConstParam.Vector3ByteSize; //12
                int grassDataByteSize = ConstParam.GrassDataByteSize; //32

                vector3BufferPtr = Marshal.AllocHGlobal(vector3ByteSize); //12bitぶんメモリの確保
                grassDataBufferPtr = Marshal.AllocHGlobal(grassDataByteSize); //bitぶんメモリの確保

                int offset = 0;
                int grassCount = LoadInt(dataBytes, intByteSize, ref offset); //バイナリからintを読み出して保存
                int divCount = LoadInt(dataBytes, intByteSize, ref offset); //バイナリからintを読み出して保存

                Vector3 minPos = LoadVector3(dataBytes, vector3ByteSize, ref vector3BufferPtr, ref offset);
                Vector3 maxPos = LoadVector3(dataBytes, vector3ByteSize, ref vector3BufferPtr, ref offset);
...
#if UNITY_EDITOR
            string fileName = GetFileName(chunkID);
            return File.ReadAllBytes(fileName);
#else
            string key = GetAssetKey(chunkID);
        ...
        private static Vector3 LoadVector3(in byte[] bytes, in int byteSize, ref IntPtr buffer, ref int offset) {
            //PtrToStructureはアンマネージドメモリからしか変換を受け付けない。
            //一旦自分作成したアンマネージドメモリをbufferで渡し、そのメモリに取り出したVector3のバイナリデータをコピーしてからVector3に変換している
