...
public class RocketLauncher_CreateConcreteFactory : ICustomizeFactory
{
    //private Handgun _handgun;
    private RocketLauncher[] _prefabs;
    private List<IObjectPool> _bulletPools;
    public RocketLauncher_CreateConcreteFactory(RocketLauncher[] prefabs, List<IObjectPool> objectPoolList)
    {
        _prefabs = prefabs;
    ...
    {
        I_Data_RocketLauncher gunData = data as I_Data_RocketLauncher;
        if (gunData == null) return null;

        RocketLauncher gun = GameObject.Instantiate(_prefabs[0]);
        IObjectPool objectPool = _bulletPools[0];

        switch (gunData.CaliberType)
        {
            case IBulletType.CaliberTypes._Rocket:
                foreach (IObjectPool pool in _bulletPools) if (pool is ObjectPool<Bullet_Rocket>) objectPool = pool;
                break;
